 ...
 * @param auth OAuth2認証クライアント
 * @param fileId PDFファイルのID
 * @returns 抽出されたテキスト情報
 */
export async function extractPdfText(auth: OAuth2Client, fileId: string): Promise<{
  status: string;
  fileId: string;
  fileName?: string;
  mimeType?: string;
 ...
 * @returns PDFファイルの一覧
 */
export async function listPdfFiles(auth: OAuth2Client, maxResults: number = 50): Promise<{
  status: string;
  files?: Array<{
    id: string;
    name: string;
