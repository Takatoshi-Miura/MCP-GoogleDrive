...
| `g_drive_check_file_modified` | 指定日時以降にファイルが変更されたかをチェック |
...
| ツール名 | 説明 | 備考 |
|----------|------|------|
| `g_drive_insert_value` | ドキュメント、スプレッドシート、スライドに値を挿入 | ドキュメント・スライド: マークダウン表形式を自動検出し、ネイティブ表として挿入 |
| `g_drive_create_chart` | 指定されたファイルにグラフを作成 | 棒グラフ、円グラフ、折れ線グラフ、散布図に対応 |
| `g_drive_create_new_element` | 指定されたファイルに新規要素を作成 | ドキュメント: API制限により不可<br>スプレッドシート: シート作成<br>スライド: スライド作成 |
| `g_drive_copy_element` | ファイルの部分をコピー | ドキュメント: 未実装<br>スプレッドシート: シートコピー<br>スライド: スライドコピー |
| `g_drive_merge_cell` | スプレッドシートの指定されたセル範囲を結合 | スプレッドシートのみ対応 |
