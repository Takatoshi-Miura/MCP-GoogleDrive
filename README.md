...
- `g_drive_copy_element` - ファイルの部分をコピー
  - **ドキュメント**: 未実装（エラーメッセージを返します）
  - **スプレッドシート**: 指定したシートを同じスプレッドシート内でコピー
  - **スライド**: 指定したスライドを同じプレゼンテーション内でコピー（タイトル変更可能）
- `g_drive_merge_cell` - スプレッドシートの指定されたセル範囲を結合
